...
const comboboxFix = () => {
  $(document).on("show.bs.dropdown", (e) => {
    if (e.relatedTarget.matches('[role="combobox"]')) {
      ...
      const listbox = document.querySelector(
        `#${combobox.getAttribute("aria-controls")}[role="listbox"]`
      );
    ...
    if (e.relatedTarget.matches('[role="combobox"]')) {
      ...
      if (listbox) {
        setTimeout(() => {
          ...
          listbox
            .querySelectorAll('.active[role="option"]')
            .forEach((option) => {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (option) {
      ...
      const combobox = document.querySelector(
        `[role="combobox"][aria-controls="${listbox.id}"]`
      );
      if (combobox) {
  ...
  document.addEventListener("change", (e) => {
    if (e.target.matches('input[type="hidden"][id]')) {
      const combobox = document.querySelector(
        `[role="combobox"][data-input-element="${e.target.id}"]`
      );
      const option = e.target.parentElement.querySelector(
        `[role="option"][data-value="${e.target.value}"]`
      );
  ...
  const selectOption = (combobox, option) => {
    const listbox = option.closest('[role="listbox"]');
    const oldSelectedOption = listbox.querySelector(
      '[role="option"][aria-selected="true"]'
    );
    ...
    if (combobox.hasAttribute("value")) {
      combobox.value =
        option.dataset.shortText ||
        option.textContent.replace(/[\n\r]+|[\s]{2,}/g, " ").trim();
    } else {
      const selectedOptionContainer = combobox.querySelector(
        "[data-selected-option]"
      );
      if (selectedOptionContainer) {
        selectedOptionContainer.textContent =
          option.dataset.shortText || option.textContent;
      } else {
...
const autoFocus = () => {
  window.addEventListener("load", () => {
    const alerts = document.querySelectorAll(
      '[data-aria-autofocus="true"][role="alert"]'
    );
...
const tabElementFix = () => {
  ...
  document.addEventListener("click", (e) => {
    if (
      e.target.matches(
        '[role="tablist"] [data-toggle="tab"], [role="tablist"] [data-toggle="pill"]'
      )
    ) {
      const tabs = e.target
        .closest('[role="tablist"]')
        .querySelectorAll('[data-toggle="tab"], [data-toggle="pill"]');
      e.preventDefault();
      $(e.target).tab("show");
      tabs.forEach((tab) => {
        tab.tabIndex = -1;
