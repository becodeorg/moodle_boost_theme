...
export default class Drawers {
   ...
   *
   * @static
   * @property {String} drawerShow See {@link event:theme_becode/drawers:show}
   * @property {String} drawerShown See {@link event:theme_becode/drawers:shown}
   * @property {String} drawerHide See {@link event:theme_becode/drawers:hide}
   * @property {String} drawerHidden See {@link event:theme_becode/drawers:hidden}
   */
  static eventTypes = {
    /**
     * An event triggered before a drawer is shown.
     *
     * @event theme_becode/drawers:show
     * @type {CustomEvent}
     * @property {HTMLElement} target The drawer that will be opened.
     */
    drawerShow: "theme_becode/drawers:show",

    /**
     * An event triggered after a drawer is shown.
     *
     * @event theme_becode/drawers:shown
     * @type {CustomEvent}
     * @property {HTMLElement} target The drawer that was be opened.
     */
    drawerShown: "theme_becode/drawers:shown",

    /**
     * An event triggered before a drawer is hidden.
     *
     * @event theme_becode/drawers:hide
     * @type {CustomEvent}
     * @property {HTMLElement} target The drawer that will be hidden.
     */
    drawerHide: "theme_becode/drawers:hide",

    /**
     * An event triggered after a drawer is hidden.
     *
     * @event theme_becode/drawers:hidden
     * @type {CustomEvent}
     * @property {HTMLElement} target The drawer that was be hidden.
     */
    drawerHidden: "theme_becode/drawers:hidden",
  };
   ...
   * @returns {module:theme_becode/drawers}
   */
  ...
  openDrawer({ focusOnCloseButton = true, setUserPref = true } = {}) {
    const pendingPromise = new Pending("theme_becode/drawers:open");
    ...
    if (
      preference &&
      !isSmall() &&
      this.drawerNode.dataset.forceopen != 1 &&
      setUserPref
    ) {
  ...
  closeDrawer({ focusOnOpenButton = true, updatePreferences = true } = {}) {
    const pendingPromise = new Pending("theme_becode/drawers:close");
   ...
   *
   * @param {module:theme_becode/drawers} comparisonInstance
   */
...
const registerListeners = () => {
    ...
    if (isSmall()) {
      ...
      drawerMap.forEach((drawerInstance) => {
        ...
        if (drawerInstance.isOpen) {
          const currentFocus = document.activeElement;
          const drawerContent = drawerInstance.drawerNode.querySelector(
            SELECTORS.DRAWERCONTENT
          );
          const shouldClose =
            drawerInstance.closeOnResize &&
            (!drawerContent || !drawerContent.contains(currentFocus));
          if (shouldClose) {
            drawerInstance.closeDrawer();
          } else {
            anyOpen = true;
  ...
  document.addEventListener("scroll", () => {
    const currentFocus = document.activeElement;
    const drawerContentElements = document.querySelectorAll(
      SELECTORS.DRAWERCONTENT
    );
    // Check if the current focus is within any drawer content.
    if (
      Array.from(drawerContentElements).some((drawer) =>
        drawer.contains(currentFocus)
      )
    ) {
      return;
    }
  ...
  window.addEventListener(
    "resize",
    debounce(closeOnResizeListener, 400, { pending: true })
  );
};
