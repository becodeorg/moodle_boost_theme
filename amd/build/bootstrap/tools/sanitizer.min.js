define("theme_becode/bootstrap/tools/sanitizer", ["exports"], function (
  _exports
) {
  Object.defineProperty(_exports, "__esModule", { value: !0 }),
    (_exports.DefaultWhitelist = void 0),
    (_exports.sanitizeHtml = function (unsafeHtml, whiteList, sanitizeFn) {
      if (0 === unsafeHtml.length) return unsafeHtml;
      if (sanitizeFn && "function" == typeof sanitizeFn)
        return sanitizeFn(unsafeHtml);
      const createdDocument = new window.DOMParser().parseFromString(
          unsafeHtml,
          "text/html"
        ),
      ...
      for (let i = 0, len = elements.length; i < len; i++) {
        ...
        const attributeList = [].slice.call(el.attributes),
          whitelistedAttributes = [].concat(
            whiteList["*"] || [],
            whiteList[elName] || []
          );
        attributeList.forEach((attr) => {
          allowedAttribute(attr, whitelistedAttributes) ||
  ...
  _exports.DefaultWhitelist = DefaultWhitelist;
  ...
  function allowedAttribute(attr, allowedAttributeList) {
    ...
    for (let i = 0, len = regExp.length; i < len; i++)
      if (regExp[i].test(attrName)) return !0;
    return !1;
  }
});
