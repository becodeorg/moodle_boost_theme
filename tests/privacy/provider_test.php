...
/**
 * Privacy tests for theme_becode.
 *
 * @package    theme_becode
 * @category   test
 * @covers     \theme_becode\privacy\provider
...
final class provider_test extends \core_privacy\tests\provider_testcase
{
    ...
    public function test_export_user_preferences(string $preference, bool $value, string $expectdescription): void
    {
        ...
        $exportedpreferences = $writer->get_user_preferences('theme_becode');
        $this->assertCount(1, (array) $exportedpreferences);
        $this->assertEquals($value, (bool) $exportedpreferences->{$preference}->value);
        $this->assertEquals(get_string($expectdescription, 'theme_becode'), $exportedpreferences->{$preference}->description);
    }
}
